    ...
    try:
        ...
        if is_attacked_by_pawn(lines, king_row, king_col) or \
           ...
           is_attacked_by_queen(lines, king_row, king_col):
            print("Checkmate! (Success)")
        else:
            print("King save! (Fail)")
...
def is_attacked_by_pawn(board, king_row, king_col):
    """Pawn โจมตีได้แนวทแยงจากล่างขึ้นบน"""
    # ตรวจสอบทแยงซ้าย (ล่างซ้าย → บนขวา)
    if king_row < len(board) - 1 and king_col > 0:
        if board[king_row + 1][king_col - 1] == 'P': 
            return True
    
    # ตรวจสอบทแยงขวา (ล่างขวา → บนซ้าย)
    if king_row < len(board) - 1 and king_col < len(board[0]) - 1:
        if board[king_row + 1][king_col + 1] == 'P':
            return True
